...
export default function Pokedex({ pokemonList }: PropType) {
  useHydrateAtoms([[nameFilterAtom, ""]] as const);
  const nameFilter = useAtomValue(nameFilterAtom);

  return (
    <div className="grid md:grid-cols-4 grid-cols-2 gap-3 lg:px-40 xl:px-56 md:px-20 px-4 col min-h-screen">
      {pokemonList &&
        pokemonList
          ...
          .map((pokemon) => (
            <div
              key={pokemon.id}
              className="flex flex-col mx-auto max-w-[250px]"
            >
              <Image
                src={
